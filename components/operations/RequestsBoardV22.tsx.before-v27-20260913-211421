...
export default function RequestsBoardV22({showToast,workerMode=false}:Props){
  ...
  async function createCharge(r:any){if(workerMode)return showToast("Создавать денежные начисления может только админ");const raw=prompt("Сумма начисления, Kč",String(r.quoted_amount||""));if(raw===null)return;const amount=Number(raw.replace?.(",",".")??raw);if(!Number.isFinite(amount)||amount<=0)return showToast("Некорректная сумма");setBusy(r.id);try{const x:any=await api("/api/admin/operations/requests",{method:"POST",body:JSON.stringify({request_id:r.id,action:"create_charge",amount})});showToast(`Начисление #${x.charge.id} создано на ${money(amount)}`);await load()}catch(e:any){showToast(e.message)}finally{setBusy(null)}}
  const counts=useMemo(()=>rows.reduce((a:any,r:any)=>{a[r.request_type]=(a[r.request_type]||0)+1;return a},{}),[rows]);
  return <div className="grid">
    <div className="card wide"><div className="space"><div><h2>📝 Заявки клиентов</h2><div className="small muted">{workerMode?"Работник может принимать, классифицировать и закрывать заявки. Денежные начисления скрыты.":"Категория ≠ статус. Сначала классифицируй, потом веди по процессу."}</div></div><button className="btn" onClick={load}>↻</button></div>
    ...
    <div className="card wide"><div className="request-board-v22">{rows.map(r=><div className={`request-card-v22 p-${r.priority||"normal"}`} key={r.id}>
      ...
      <div className="request-meta-v22"><span>Подтип: <b>{subtypeLabel(r.request_type,r.request_subtype)}</b></span>{r.preferred_date&&<span>Дата: <b>{r.preferred_date}</b></span>}{!workerMode&&r.quoted_amount&&<span>Цена: <b>{money(r.quoted_amount)}</b></span>}{!workerMode&&r.resolved_charge_id&&<span>Начисление: <b>#{r.resolved_charge_id}</b></span>}</div>
      ...
      <div className="row" style={{marginTop:10,flexWrap:"wrap"}}><button className="btn" disabled={busy===r.id} onClick={()=>update(r.id,{status:"in_progress",assign_to_me:true})}>👤 Взять в работу</button>{!workerMode&&["repair_request","battery_request","accessory_request","payment_request","contract_request","other_request"].includes(r.request_type)&&!r.resolved_charge_id&&<button className="btn primary" disabled={busy===r.id} onClick={()=>createCharge(r)}>💰 Выставить начисление</button>}<button className="btn" disabled={busy===r.id} onClick={()=>update(r.id,{status:"closed"})}>✓ Закрыть</button></div>
