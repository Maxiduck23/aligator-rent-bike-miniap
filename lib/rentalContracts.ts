...
export function validateBatterySlot(value: unknown): BatterySlot {
  ...
  if (mode === 'existing') {
    return { mode, battery_id: positiveInteger(raw.battery_id, 'battery_id') };
  }

  if (mode === 'create' || mode === 'temporary') {
    return {
      mode,
      type_id: positiveInteger(raw.type_id, 'type_id'),
...
export function validateRentalContractInput(value: unknown): RentalContractInput {
  ...
  if (!Number.isInteger(extraBatteryCount) || extraBatteryCount < 0) {
    throw new Error('extra_battery_count должен быть целым числом от 0');
  }

  const chargerQuantity = Number(raw.charger_quantity || 1);
  if (!Number.isInteger(chargerQuantity) || chargerQuantity < 0 || chargerQuantity > 10) {
    throw new Error('charger_quantity должен быть целым числом от 0 до 10');
  }

  const initialAmount = Number(raw.initial_payment?.amount || 0);
  if (!Number.isFinite(initialAmount) || initialAmount < 0) {
    throw new Error('initial_payment.amount не может быть отрицательным');
  }
  ...
  return {
    ...
    initial_payment: {
      ...
      method: String(raw.initial_payment?.method || 'manual').trim() || 'manual',
    },
