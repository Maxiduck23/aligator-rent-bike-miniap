...
- Reply-keyboard CRM button in the bot should be plain text; it sends an inline WebApp button like `/crm`, because inline WebApp buttons pass Telegram initData more reliably.
- Payment rule UI now explicitly explains 29/30/31 day behavior: backend charge generation clamps these to the last day of short months.
- Invite API now normalizes `TELEGRAM_BOT_USERNAME` by removing leading `@`, returns both `invite_key` and `link`, and validates invite key format.
